...
export async function resolveNode(state: AgentRunState): Promise<Partial<AgentRunState>> {
  await setStep(state.reportId, 'resolving')
  const raw = await chat(RESOLVE_PROMPT, `Project/developer query: "${state.projectInput}"`)
  const { resolvedName, searchQueries } = parseJSON(raw, ResolveOutputSchema)
  return { resolvedName, searchQueries }
}

export async function searchNode(state: AgentRunState): Promise<Partial<AgentRunState>> {
...
export async function extractAndScoreNode(state: AgentRunState): Promise<Partial<AgentRunState>> {
  ...
  const { signals, credibilityScore, riskLevel, summary } = parseJSON(raw, ExtractOutputSchema)
  const sources: Source[] = state.searchResults.map(r => ({ title: r.title, url: r.url }))

  return { signals, credibilityScore, riskLevel, summary, sources }
}

export async function saveNode(state: AgentRunState): Promise<Partial<AgentRunState>> {
