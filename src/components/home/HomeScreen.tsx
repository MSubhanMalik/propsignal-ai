    ...
    <div className="relative">
      <div className="bg-white rounded-2xl border-2 border-foreground p-6 shadow-[6px_6px_0px_oklch(0.105_0.038_265)]">
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <p className="font-display font-bold text-base text-foreground">Bahria Town Karachi</p>
            <p className="text-xs text-muted-foreground mt-0.5">Searched 2 queries · 12 results</p>
          </div>
          <div className="flex flex-col items-end gap-1.5">
            <span className="font-display font-bold text-3xl text-amber-600 leading-none">72</span>
          ...
          {MOCK_SIGNALS.map((s, i) => (
            <div key={i} className="flex items-center gap-2.5 text-xs">
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${
                s.neg
                  ? s.icon === '!' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600'
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 pt-8 pb-20 items-center">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-lg leading-relaxed mb-10 max-w-md">
              Enter any developer or project name. PropSignal scans public sources —
              news, court records, forums — and scores credibility in minutes.
            </p>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {r.credibility_score != null && (
                        <span className="font-display font-bold text-base text-foreground">{r.credibility_score}</span>
                      )}
                      {r.risk_level
                        ? <RiskBadge level={r.risk_level as RiskLevel} />
                        : <span className="text-xs text-muted-foreground capitalize">{r.status}</span>
                      }
                      <span className="text-muted-foreground group-hover:text-primary text-sm transition-colors">→</span>
                    </div>
                  </button>
                </div>
              ))}
